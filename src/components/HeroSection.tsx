...
export default function HeroSection() {
  ...
  return (
    ...
    <section className="relative w-full h-[80vh] md:h-[85vh] lg:h-[90vh] flex flex-col justify-center bg-black overflow-visible font-sans">
      ...
      {/* --- 3. THE SPLIT OVERLAP WIDGET CARD --- */}
      {/* absolute positioning anchors it directly to the bottom edge.
          translate-y-1/2 forces exactly 50% of the box to hang over into the next component section below. */}
      <div className="absolute bottom-0 left-0 right-0 z-30 transform translate-y-1/2 px-4 md:px-12 lg:px-24">
        <motion.div
          initial="hidden"
          animate="visible"
          variants={widgetVariants}
          className="max-w-6xl mx-auto bg-[#121212] border border-zinc-800 rounded-2xl p-6 md:p-8 shadow-[0_25px_60px_rgba(0,0,0,0.8)] relative"
        >
          ...
          <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
              ...
              <input 
                ...
                required
                className="w-full bg-zinc-900 border border-zinc-800 text-white placeholder-zinc-500 rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-[#f59e0b] transition-colors"
              />
              ...
              <select 
                className="w-full bg-zinc-900 border border-zinc-800 text-zinc-300 rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-[#f59e0b] transition-colors appearance-none cursor-pointer"
                defaultValue=""
              >
            ...
            <div className="flex justify-center pt-2">
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                type="submit"
                className="w-full sm:w-auto bg-[#f59e0b] hover:bg-[#fbbf24] text-black font-sans font-bold text-sm uppercase tracking-widest px-10 py-3.5 rounded-lg transition-colors duration-200 shadow-[0_4px_20px_rgba(245,158,11,0.2)]"
              >
                Book Consultation
              </motion.button>
            </div>
